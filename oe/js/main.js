...
if (canvas) {
  ...
  const data = {
    labels: ['Interactive E-Magazine', 'Websites', 'E-Newsletter', 'Web TV', 'Social'],
    datasets: [{
      label: 'Audience',
      data: [71025, 466415, 55674, 80974, 105139],
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const navItems = nav ? nav.querySelectorAll('a') : [];
  if (burger && nav) {
  ...
  if (navItems.length > 0 && burger && nav) {
    navItems.forEach(el => {
...
__webpack_require__.r(__webpack_exports__);
(() => {
  const e = document.querySelector(".cookies-infobar");
  const n = document.querySelector("#cookies-infobar-close");
  const t = document.querySelector("#false");
  if (!e || !n || !t) {
    // якщо блок з куками або кнопки відсутні, просто виходимо
    return;
  }
  if (o()) {
    i();
  ...
  function c(days) {
    const n = new Date();
    const t = n.getTime() + 864e5 * days;
    n.setTime(t);
    document.cookie = "cookieInfoHidden=1; expires=" + n.toUTCString() + "; path=/";
  }
...
document.onreadystatechange = function () {
  ...
  if (document.readyState !== "complete") {
    ...
    if (siteContainer) siteContainer.classList.add('site-container--hidden');
  } else {
    if (loader) loader.style.display = 'none';
    if (siteContainer) siteContainer.classList.remove('site-container--hidden');
  }
};
    ...
    if (buttons.length > 0 && contents.length > 0) {
      buttons.forEach(button => {
        button.addEventListener('click', () => {
          ...
          const targetContent = tabComponent.querySelector(`.tab-component__content[data-content="${tab}"]`);
...
am5.ready(function () {
  ...
  if (!mapEl) return; // якщо елементу немає, пропускаємо весь блок
